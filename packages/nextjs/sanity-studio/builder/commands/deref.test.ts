...
type SchemaForDeref = {
  category: {
    _id: string;
    _type: "category";
    products: Array<{ _type: "reference"; _ref: string; [_referenced]: "product" }>;
  };
  product: {
    _id: string;
    _type: "product";
...
const q = createGroqBuilder<{ TSchema: SchemaForDeref; referenced: typeof _referenced }>();
...
describe("deref", () => {
  ...
  it("", () => {
    ...
    expectType<ExtractScope<typeof res>>().toStrictEqual<Array<SchemaForDeref["product"]>>();
  ...
  it("", () => {
    ...
    expectType<ExtractScope<typeof res>>().toStrictEqual<SchemaForDeref["category"]>();
    expect(q).toMatchObject({
